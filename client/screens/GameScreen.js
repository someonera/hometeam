...
export function GameScreen({ route, navigation }) {
  ...
  const [
    newGame,
    { loading: newLoading, error: newError, data: newData },
  ] = useMutation(NEW_GAME);
  const {
    loading: filterLoading,
    error: filterError,
    data: filterData,
  } = useQuery(GET_FILTERED_TASKS, {
    variables: { mon: mondaysDate, sun: sundaysDate },
  });
  ...
  useEffect(() => {
    if (
      ...
      !gameData.getGame &&
      filterData &&
      filterData.getAllTasks &&
      !game
    ) {
      const removeType = filterData.getAllTasks.map(
        (item) => (item = { taskName: item.taskName })
      );

      if (!game) {
        newGame({
          variables: {
            startDate: mondaysDate,
            endDate: sundaysDate,
            gameTasks: removeType,
          },
        }).then((newData) => {
      ...
      {game && (
        <View>
          ...
          >
            ...
            >
              <Text style={gameStyles.awayScore}>
          ...
          <View style={{ marginTop: 20 }}>
            {game.gameTasks.length === game.score ? (
              <Icon name={"award"} size={200} color={"gold"} />
            ) : null}
          </View>
        </View>
      )}
