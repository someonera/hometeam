...
export function StatsScreen({route, navigation}) {
  ...
  const {loading, error, data} = useQuery(GET_PAST_GAMES, {variables: {endDate: sundaysDate}})
console.log(data)
...
if (error) return <Text>`Error! ${error.message}`</Text>;
  ...
  return (
    ...
    {data.getAllPastGames.map(({score, gameTasks, endDate, startDate}) => (
      <ListItem key={score}>
        <Icon name={(gameTasks.length-score) < score ? "award" : "meh"} 
              size={50} 
              color={(gameTasks.length-score) < score ? "gold" : "red"}
            />
        <ListItem.Content>
          <Text style={styles.listItem}>{moment(startDate).format('MMMM Do')} - {moment(endDate).format('MMMM Do')} </Text>
          <Text style={{fontSize: 25, color: "green"}}>Home {score} : {gameTasks.length - score} Away</Text>
