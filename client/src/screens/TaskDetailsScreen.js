...
export function TaskDetailsScreen({ route, navigation }) {
  const [
    addTask,
    { loading: addLoad, error: addErr, data: addData },
  ...
  ] = useMutation(ADD_USER_TO_TASK);

  const { taskName, id, startDate, interval, taskWho } = route.params;

  const [title, setTitle] = React.useState(
    taskName === "" ? "New Task Name" : taskName.taskName
  );
  const [taskId, setTaskId] = React.useState(id === "" ? "" : id.id);
  const [taskStartDate, setTaskStartDate] = React.useState(
    startDate === "" ? moment() : startDate.startDate
  );
  const [taskInterval, setTaskInterval] = React.useState(
    interval === "" ? "7" : interval.interval
  );
  const [taskAlloc, setTaskAlloc] = React.useState(
    taskWho === "" ? "Parent" : taskWho.taskWho
  );
    ...
    <Card containerStyle={{ height: "95%" }}>
      ...
      >
        <Picker
          ...
          onValueChange={(itemValue) => setTaskInterval(itemValue)}
        >
      ...
      >
        ...
        <Button
          ...
          onPress={() => navigation.navigate("Our Goals")}
        ></Button>
      </View>
    </Card>
  );
